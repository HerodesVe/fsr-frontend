...
import type { 
  ...
  UploadedDocument,
  DocumentosAdministradoConVariacion,
} from '@/types/conformidad.types';

interface StepDocumentosAdministradoConVariacionProps {
...
// Documentos requeridos al administrado - CON VARIACIÓN
const DOCUMENTOS_ADMINISTRADO_CON_VARIACION = [
  ...
  { key: 'cuaderno_obra_variaciones', label: 'Copia de la Sección del Cuaderno de Obra que Acredita Variaciones', required: true },
...
export default function StepDocumentosAdministradoConVariacion({
  formData,
  errors,
  elaboracionId,
  uploadedDocuments,
  ...
  onFileUpload,
  onDownloadDocument
}: StepDocumentosAdministradoConVariacionProps) {
  
  const documentos = formData.documentos_administrado;

  const handleDocumentoChange = useCallback((key: keyof DocumentosAdministradoConVariacion, files: File[]) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600" style={{ fontFamily: 'Inter, sans-serif' }}>
          Documentos requeridos al administrado para la elaboración con variación.
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="space-y-4">
          {DOCUMENTOS_ADMINISTRADO_CON_VARIACION.map((doc) => (
            <div key={doc.key} className="p-4 border border-gray-200 rounded-lg hover:border-teal-300 transition-colors">
              ...
              <FileUpload
                ...
                value={documentos[doc.key as keyof DocumentosAdministradoConVariacion] || []}
                onChange={(files) => handleDocumentoChange(doc.key as keyof DocumentosAdministradoConVariacion, files)}
                accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"
                multiple
                onUpload={onFileUpload}
                documentKey={`docs_administrado_cv.${doc.key}`}
                anteproyectoId={elaboracionId}
                uploadedFiles={uploadedDocuments
                  .filter(d => d.key === `docs_administrado_cv.${doc.key}`)
