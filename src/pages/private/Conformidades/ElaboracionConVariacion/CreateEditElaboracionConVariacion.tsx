...
import {
  ...
  createEmptyDocumentosAdministradoConVariacion,
  createEmptyDocumentosFSRConVariacion,
} from '@/types/conformidad.types';
...
import { StepAntecedentesElaboracion, StepDocumentosAdministradoConVariacion, StepDocumentosFSRConVariacion } from './Steps';
import { ResumenElaboracionConVariacion } from './components';

export default function CreateEditElaboracionConVariacion() {
  const { id } = useParams();
  ...
  const [formData, setFormData] = useState<ElaboracionConVariacionFormData>({
    selectedClient: null,
    nombre_proyecto: '',
  ...
  useEffect(() => {
    setHeader(
      isEdit ? 'Editar Elaboración Con Variación' : 'Nueva Elaboración Con Variación',
      'Elaboración de conformidad de obra con variaciones'
    );
  ...
  const handleInputChange = useCallback((field: keyof ElaboracionConVariacionFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate('/dashboard/elaboracion-conformidad-con-variacion');
    } catch (error) {
  ...
  const renderStepContent = () => {
    ...
    switch (actualStep) {
      case 0: // Administrado
        return (
          <StepAdministrado
            ...
            clients={clients || []}
            errors={errors}
            onInputChange={(field: string, value: any) => handleInputChange(field as keyof ElaboracionConVariacionFormData, value)}
      ...
      case 6: // Documentos Administrado
        return (
          <StepDocumentosAdministradoConVariacion
            formData={formData}
            errors={errors}
            elaboracionId={elaboracionId || 'new'}
            uploadedDocuments={uploadedDocuments}
      ...
      case 7: // Documentos FSR
        return (
          <StepDocumentosFSRConVariacion
            formData={formData}
            errors={errors}
            elaboracionId={elaboracionId || 'new'}
            uploadedDocuments={uploadedDocuments}
    ...
    <div className="p-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="flex items-center justify-between pt-6 border-t border-gray-200">
              ...
              <div className="flex items-center gap-4">
                <Button
                  variant="bordered"
                  onClick={() => navigate('/dashboard/elaboracion-conformidad-con-variacion')}
        ...
        <div className="lg:col-span-1">
          <ResumenElaboracionConVariacion
            formData={formData}
