...
export default function ElaboracionConVariacionList() {
  const navigate = useNavigate();
  const { setHeader } = useHeaderStore();
  const [searchTerm, setSearchTerm] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const elaboraciones: any[] = [];
  ...
  const handleCreateNew = () => {
    navigate('/dashboard/elaboracion-conformidad-con-variacion/create');
  };

  const filteredElaboraciones = useMemo(() => {
    if (!searchTerm) return elaboraciones;
    
    return elaboraciones.filter((item: any) => 
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {isLoading ? (
      ...
      ) : filteredElaboraciones.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600 mb-6" style={{ fontFamily: 'Inter, sans-serif' }}>
            Comienza creando una nueva elaboración de conformidad con variación
          </p>
          <Button
            onClick={handleCreateNew}
            style={{ backgroundColor: 'var(--primary-color)' }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredElaboraciones.map((item: any) => (
            <div 
              key={item.id}
              className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => navigate(`/dashboard/elaboracion-conformidad-con-variacion/${item.id}`)}
            >
