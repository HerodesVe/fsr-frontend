...
import type { LicenciaFuncionamientoFormData, FormStep, UploadedDocument } from '@/types/licenciaFuncionamiento.types';

interface ResumenLicenciaFuncionamientoProps {
...
export const ResumenLicenciaFuncionamiento: React.FC<ResumenLicenciaFuncionamientoProps> = ({
  formData,
  currentStep,
  steps,
  licenciaId,
  ...
  const documentosObligatorios = [
    { 
      ...
      completed: isDocumentCompleted('vigencia_poder', formData.vigencia_poder) 
    },
    { 
      name: 'HRPU (Hoja Resumen y Predio Urbano)', 
      completed: isDocumentCompleted('hrpu', formData.hrpu) 
    },
    { 
      name: 'Declaratoria de Fábrica', 
      completed: isDocumentCompleted('declaratoria_fabrica', formData.declaratoria_fabrica) 
    },
    { 
      name: 'Certificado de Pozo a Tierra', 
      completed: isDocumentCompleted('certificado_pozo_tierra', formData.certificado_pozo_tierra) 
    },
    // Documentos condicionados por nivel de riesgo
    ...(formData.nivel_riesgo === 'Alto' || formData.nivel_riesgo === 'Muy Alto' ? [
      { 
        name: 'Planos de Arquitectura', 
        completed: isDocumentCompleted('planos_arquitectura', formData.planos_arquitectura) 
      },
      { 
        name: 'Planos de Seguridad', 
        completed: isDocumentCompleted('planos_seguridad', formData.planos_seguridad) 
      },
      { 
        name: 'Planos Eléctricos', 
        completed: isDocumentCompleted('planos_electricos', formData.planos_electricos) 
      },
      { 
        name: 'Plan de Seguridad', 
        completed: isDocumentCompleted('plan_seguridad', formData.plan_seguridad) 
      },
    ] : []),
    { 
      name: 'Acta de Inspección', 
      completed: isDocumentCompleted('acta_inspeccion', formData.acta_inspeccion) 
    },
    { 
      name: 'Certificado ITSE', 
      completed: isDocumentCompleted('certificado_itse', formData.certificado_itse) 
    },
    { 
      name: 'Licencia de Funcionamiento', 
      completed: isDocumentCompleted('licencia_funcionamiento', formData.licencia_funcionamiento) 
    },
    { 
      name: 'Acta de Entrega Firmada', 
      completed: isDocumentCompleted('acta_entrega_firmada', formData.acta_entrega_firmada) 
    },
  ];
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
      ...
      {(formData.giro_negocio || formData.direccion_local) && (
        <div>
          ...
          <div className="space-y-2">
            {formData.giro_negocio && (
              <div className="text-xs text-gray-600" style={{ fontFamily: 'Inter, sans-serif' }}>
                <span className="font-medium">Giro:</span> {formData.giro_negocio}
              </div>
            )}
            {formData.direccion_local && (
              <div className="text-xs text-gray-600" style={{ fontFamily: 'Inter, sans-serif' }}>
                <span className="font-medium">Dirección:</span> {formData.direccion_local}
              </div>
            )}
            {formData.nivel_riesgo && (
              <div className="text-xs text-gray-600" style={{ fontFamily: 'Inter, sans-serif' }}>
                <span className="font-medium">Nivel de Riesgo:</span> {formData.nivel_riesgo}
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {formData.visitas.map((visita, index) => (
              <div key={visita.id} className="text-xs text-gray-600" style={{ fontFamily: 'Inter, sans-serif' }}>
                <span className="font-medium">Visita {index + 1}:</span> {visita.fecha} - {visita.estado_local}
              </div>
            ))}
          </div>
        </div>
